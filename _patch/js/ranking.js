...
async function loadAll() {
  const [usersRes, predsRes, champsRes, matchesRes] = await Promise.all([
    ...
    supabase.from('predictions').select('user_id, match_id, home_score_pred, away_score_pred, points_earned'),
...
function computeStreaks() {
  ...
  // VA PARA ATRÁS: top 3 by sum ASC (must have predicted at least one)
  const cold = eligibleHot.slice().sort((a, b) => a.sum - b.sum).slice(0, 3)
...
function renderStreaks() {
  ...
  if (!container) {
    // No streaks container in markup — skip
    return
  }
...
async function showUserDetail(userId) {
  ...
  const sortedPreds = (preds || []).slice()
    // Only show LOCKED or FINISHED — never another user's still-editable picks.
    .filter(p => {
      const m = p.match
      if (!m) return false
  ...
  const html = `
    ...
    <div class="detail-body">
      ${champHtml}
      <h4>Predicciones <span class="count">${sortedPreds.length} partidos</span></h4>
      <div class="history-list">${rows || '<div class="empty-state"><p>Sin predicciones todavía.</p></div>'}</div>
    </div>
  `
